...
namespace parser_input
{
    MH2O::MH2O(long position, utility::BinaryStream *reader) : AdtChunk(position, reader)
    {
        ...
        for (int y = 0; y < 16; ++y)
            for (int x = 0; x < 16; ++x)
            {
                std::unique_ptr<MH2OHeader> header(reader->AllocateAndReadStruct<MH2OHeader>());
                ...
                Blocks[y][x]->Header = std::move(header);

                // jump to (and read) the data struct
                reader->SetPosition(offsetFrom + Blocks[y][x]->Header->DataOffset);
                Blocks[y][x]->Data.reset(reader->AllocateAndReadStruct<MH2OData>());
