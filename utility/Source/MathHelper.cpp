...
namespace utility
{
    ...
    bool MathHelper::FaceTooSteep(const Vertex &a, const Vertex &b, const Vertex &c, float degrees)
    {
        return CalculateTriangleNormal(a, b, c).Z <= cosf(PI * degrees / 180.f);
    }

    float MathHelper::InterpolateHeight(const Vertex &a, const Vertex &b, const Vertex &c, float x, float y)
    {
