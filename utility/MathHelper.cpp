...
namespace math
{
bool MathHelper::FaceTooSteep(const Vector3 &a, const Vector3 &b, const Vector3 &c, float degrees)
{
    return CalculateTriangleNormal(a, b, c).Z <= cosf(PI * degrees / 180.f);
}

float MathHelper::InterpolateHeight(const Vector3 &a, const Vector3 &b, const Vector3 &c, float x, float y)
{
...
//BoundingBox::BoundingBox(const Vector3 &a, const Vector3 &b, const Vector3 &c)
...
void Convert::WorldToAdt(const Vector3 &Vector3, int &adtX, int &adtY, int &chunkX, int &chunkY)
{
    ...
    chunkX = static_cast<int>((nwCornerY - Vector3.Y) / MeshSettings::AdtChunkSize);
    chunkY = static_cast<int>((nwCornerX - Vector3.X) / MeshSettings::AdtChunkSize);
...
void Convert::VerticesToRecast(const std::vector<Vector3> &input, std::vector<float> &output)
{
...
void Convert::VerticesToWow(const float *input, int Vector3Count, std::vector<Vector3> &output)
{
