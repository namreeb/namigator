...
{
    public:
        static bool FaceTooSteep(const Vector3 &a, const Vector3 &b, const Vector3 &c, float degrees);
        static float InterpolateHeight(const Vector3 &a, const Vector3 &b, const Vector3 &c, float x, float y);
...
class Convert
{
    public:
        ...
        static void WorldToAdt(const Vector3 &vertex, int &adtX, int &adtY, int &chunkX, int &chunkY);
        ...
        static void VerticesToRecast(const std::vector<Vector3> &input, std::vector<float> &output);

        static void VertexToWow(const float *input, Vector3 &output);
        static void VerticesToWow(const float *input, int vertexCount, std::vector<Vector3> &output);
};
}