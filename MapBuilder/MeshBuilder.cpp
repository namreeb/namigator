...
void SerializeTileQuadHeight(const parser::AdtChunk *chunk, int tileX, int tileY, utility::BinaryStream &out)
{
    ...
    utility::BinaryStream result(1 + 4*2 + width*width + sizeof(float)*MeshSettings::QuadValuesPerTile);
...
bool MeshBuilder::BuildAndSerializeWMOTile(int tileX, int tileY)
{
    ...
    const math::BoundingBox tileBounds({ config.bmax[2], config.bmax[0], config.bmin[1] }, { config.bmin[2], config.bmin[0], config.bmax[1] });
