...
BVHConstructor::BVHConstructor(const fs::path& outputPath) : m_outputPath(outputPath), m_shutdown(false)
{
...
fs::path BVHConstructor::InternalAddFile(const fs::path& mpq_path)
{
    ...
    // compute sha256 checksum of path in mpq to give a unique name without symbols
    std::vector<unsigned char> hash(picosha2::k_digest_size);

    auto const mpq_path_str = mpq_path.string();
    std::vector<char> filename_vect;
    std::copy(mpq_path_str.begin(), mpq_path_str.end(), std::back_inserter(filename_vect));
...
fs::path BVHConstructor::AddTemporaryObstacle(std::uint32_t id, const fs::path& mpq_path)
{
    std::lock_guard<std::mutex> guard(m_mutex);
    return m_outputPath / "BVH" / (m_temporaryObstacles[id] = InternalAddFile(mpq_path).string());
}
...
void BVHConstructor::Shutdown()
{
    ...
    std::vector<std::pair<std::string, std::string>> serialized(m_files.begin(), m_files.end());
    m_files.clear();
    ...
    for (auto const& entry : serialized)
        out << static_cast<std::uint32_t>(entry.first.length()) << entry.first << static_cast<std::uint32_t>(entry.second.length())
        << entry.second;

    std::vector<std::pair<std::uint32_t, std::string>> obstacles(m_temporaryObstacles.begin(), m_temporaryObstacles.end());
